...
export function UserFormDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <UserFormDialogBody
        key={`${mode}-${user?.id ?? "none"}-${open ? "open" : "closed"}`}
        mode={mode}
        user={user}
        roles={roles}
...
function createInitialForm(mode: "create" | "edit", user: User | null) {
  if (mode === "edit" && user) {
    return {
...
function UserFormDialogBody({
  ...
  const [form, setForm] = useState<Record<string, string>>(() => createInitialForm(mode, user));
